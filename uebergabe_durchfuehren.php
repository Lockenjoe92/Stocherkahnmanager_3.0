...
if ($Parser == NULL){
    ...
    $HTML .= zurueck_karte_generieren(TRUE, 'Fehler bei der Durchf&uuml;hrung der &Uuml;bergabe!', 'termine.php');
}
...
function do_uebergabe_parser($UebergabeID){
    ...
    if(intval($UebergabeID > 0)){
        if(isset($_POST['uebergabe_durchfuehren'])){
            ...
            $Ergebnis = uebergabe_durchfuehren($UebergabeID, $_POST['schluessel'], $_POST['gezahlter_betrag'], $AndererPreis, $Gratisfahrt, $Status);
...
function formular_do_uebergabe($UebergabeID){
    ...
    $DurchfuehrungTableHTML = table_row_builder(table_header_builder('Schlüssel auswählen').table_data_builder(dropdown_verfuegbare_schluessel_wart('schluessel', $Uebergabe['wart'])));
    ...
    $DurchfuehrungTableHTML .= table_row_builder(table_header_builder(form_button_builder('uebergabe_durchfuehren', 'Durchführen', 'action', 'send', '')."&nbsp;".button_link_creator('Abbrechen', './wartwesen.php', 'arrow_back', '')).table_data_builder(''));
    $DurchfuehrungTable = table_builder($DurchfuehrungTableHTML);
    $DurchfuehrungTableHTML = form_builder($DurchfuehrungTable, '#', 'post');
