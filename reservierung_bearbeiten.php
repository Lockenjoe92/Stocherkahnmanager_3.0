...
function reservierung_bearbeiten_seiteninhalt($Mode, $ResID, $Parser){
    ...
    if ($Parser['success'] === NULL){
        ...
        //Zeiten ändern
        $HTML .= karte_zeiten_aendern($ResID);
...
function parse_res_bearbeiten($Res){
    ...
    if (isset($_POST['zeiten_veraendern'])){
        ...
        $ResBearbeiten = reservierung_bearbeiten($Res, $AngabeBeginn, $AngabeEnde);
...
function karte_zeiten_aendern($ResID){
    ...
    $FormTable = form_builder($FormTable, './reservierung_bearbeiten.php?id='.$ResID.'', 'post', '', '');
    $HTML .= $FormTable;
